...
1. **Base heads** — one bald, faceless mannequin per gender × angle (6 images), made by
   *editing* `reference-head.png` (cropped straight out of `App-reference.png`) down to a bald
   ...
   The `front` angle is a **three-quarter** turn, not a dead-on shot: it is the hero image on
   the card and the style screen, and the hero in `App-reference.png` is angled. The mockup has
   no straight-on view anywhere, so the three angles are three-quarter, profile and back.
...
app grades that render to whichever shade the user picks (`src/lib/colorGrade.ts`). See `lib/prompts.mjs` — `HOUSE_STYLE` is the consistency contract and should stay
...
Compare `assets/mannequins/low-taper-fade/male-front.png` against the hero shot in
...
```bash
npm run mannequins -- --yes        # 138 style images + 6 heads, roughly $6
```
...
assets/mannequins/
  ...
  low-taper-fade/male-front.png …   <styleId>/<gender>-<angle>.png
  manifest.json                     styleId -> gender -> angle -> { file, url, prompt }
  index.html                        contact sheet for review
```
...
```bash
npm run mannequins -- --sheet --gender male --style crew-cut
# … ✓ crew-cut male -> crew-cut/male-sheet.png (+4 views)
# App renders: src/api/mannequinRenders.generated.ts — 64 view(s) across 16 style(s) (updated)
```
...
the Railway database, and it keeps the exact prompt used for each image so a single style can be
...
Note that `Hairstyle` currently has one `imageUrl`, while this produces gender × angle variants.
Wiring that up is a catalog-shape change (`imageUrl` → per-gender, per-angle urls) and belongs
with the backend work, not here.
...
Roughly $0.04 per image at nano-banana list price, so the full catalog is about **$6**. Check
