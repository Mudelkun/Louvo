 ...
 * passes through a Hairify server, which is what keeps the promise
...
export function captionFor(hairstyleName: string, url: string): string {
  const opening = `Trying the ${hairstyleName} with Hairify ✂️`;
  if (!url) return `${opening}\nWant to see how a haircut looks on you? Try Hairify.`;
...
export async function createShareLink(
  ...
  const local: ShareLink = {
    ...
    title: `See yourself with a ${look.hairstyleName} — Hairify`,
