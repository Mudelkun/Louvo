...
const CATALOG_SOURCE_COPY: Record<ReturnType<typeof catalogSource>, string> = {
  api: 'Hairstyles are loaded from the Louvo catalog API and their imagery from the CDN.',
...
const GENERATION_COPY: Record<ReturnType<typeof generationSource>, string> = {
  server:
    'Generating a preview uploads your photo to Louvo, which passes it to the image provider and deletes it as soon as the preview is made. ' +
  ...
  direct:
    'Generating a preview sends your photo straight from this phone to the image provider; it is not stored on any Louvo server.',
        ...
        <View>
          ...
          <View style={styles.group}>
            ...
            <LinkRow
              ...
              hint="The agreement between you and Louvo"
        ...
        <View style={styles.about}>
          ...
          <Text style={[type.caption, { color: colors.muted }]}>
            {generationSource() === 'server'
              ? 'Previews are generated by the Louvo backend, which keeps the generator key and runs the job even if you close the app.'
          ...
          <Text style={[type.caption, { color: colors.muted }]}>
            {screenCaptureSource() === 'blocked'
              ? 'Screenshots and screen recordings of Louvo are turned off, so hairstyle renders stay in the app. Android refuses the capture outright; on iPhone the picture comes out blank. Sharing a look still sends a proper image.'
          ...
          <Text style={[type.caption, { color: colors.muted }]}>
            {shareSource() === 'api'
              ? 'Shared looks carry a Louvo link that opens the app if it is installed and the store if it is not, and the shares it brings in are counted against it.'
